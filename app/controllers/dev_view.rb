class ViewsController < ApplicationController
  ...
  def url_dashboard
     ...
     start_date = params[:start_date]
     end_date = params[:end_date]
     dvm = DailyViewKeyValue.where("date >=  ? AND date <= ? AND key like ? AND view_id = ?", start_date, end_date, "%#{params[:url]}%", params[:view_id])
    dvm = {
    ...
    def top_entries(type, id, collection, args = {})
      ...
      pct = ds.value.to_f / total_sessions 
        if pct >= percentile && ds.key != "(not set)"
         c_name=NewPipelineApi.where("url=? or url=? or url=? or url=?", "https://www.#{ds.key}","http://www.#{ds.key}","http://#{ds.key}", "https://#{ds.key}").last 
         if !c_name.nil?
          ss <<  ds.key
