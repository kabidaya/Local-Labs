class NewPipelineApi < ActiveRecord::Base

def self.pull_all
(1..1000).each do |n|
 p "pageno_#{n}"
 p "Date:#{(Date.today-1).strftime('%Y-%m-%d')}"
 response = HTTParty.get("https://pipeline-api.locallabs.com/api/v1/organizations?min_updated_at=#{(Date.today-1).strftime('%Y-%m-%d')}&page=#{n}",headers: {"Authorization" => "TJX5h5NUpQRreu6z9DEh"})
